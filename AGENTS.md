...
- 已从 EdgeOne 生产日志定位并修复腾讯 SDK CommonJS/ESM 默认导出不兼容导致的 Cloud Function 冷启动 502；同时适配 EdgeOne 反向代理保留的 Host/Forwarded Host，避免自定义域名同源请求被误拒绝。
...
| 日期 | 变更内容 |
|------|----------|
| 2026-08-04 | 修复 EdgeOne 打包腾讯 OCR SDK 时默认导出为空造成的 `/api/ocr` 冷启动 502，并适配反向代理 Host 的同源判断；why：让自定义域名请求能进入 PIN、配置及腾讯 OCR 调用流程，而非在 0 ms 初始化阶段崩溃或被误判为跨域 |
